...
func smallDq(folderPath string) Interface {
...
func largeDq(folderPath string) Interface {
	dqLogf := func(level LogLevel, f string, args ...interface{}) {
		Logf(f, args...)
	}
	dq := NewWithDiskSpace(
...
func setupDq(folderPath string) {
	fmt.Println("## Creating a diskbuffer...")

	os.MkdirAll(folderPath, 0755)
}

func NewDiskqueue() (Interface, error) {
	fmt.Println("## Creating a diskbuffer...")
	dqLogf := func(level LogLevel, f string, args ...interface{}) {
		Logf(f, args...)
	}
...
func writeData(diskQueue Interface) {
	msgSize := 36000
	msg := make([]byte, msgSize)
	// err := diskQueue.Put(msg)

	for i := 0; i < 23*2; i++ {
		randSize := rand.Int63n(990000) + 1000
		rndMsg := make([]byte, randSize)
		fmt.Println(randSize)
		err := diskQueue.Put(rndMsg)
		if err != nil {
			fmt.Println(err)
			return
		}
	...
	// err = diskQueue.Put(make([]byte, 8879))
	err = diskQueue.Put(make([]byte, 8880))
	if err != nil {
		fmt.Println(err)
		return
	}
...
func printStats(diskQueue Interface) {
	fmt.Println("STATS:")
	fmt.Println("Put", diskQueue.Depth(), "messages")
	readMetaData()
}

func readMetaData() {
	fn := "./nsq_dq_test/test_dq.diskqueue.meta.dat"
	...
	var depth, readFileNum, readMessages, readPos, writeFileNum, writeMessages, writePos int64
	...
	if err != nil {
		fmt.Print("ERROR: cannot scan file: ")
		fmt.Println(err)
		return
	}
	...
	statStr := fmt.Sprintf(
		...
		depth, readFileNum, readMessages, readPos, writeFileNum, writeMessages, writePos)
	fmt.Println(statStr)
}

func truncateDQFile(fileNum int64) {
	dqFn := fmt.Sprintf("./nsq_dq_test/test_dq.diskqueue.%06d.dat", fileNum)
	os.Truncate(dqFn, 500)
}

func startWriteData(diskQueue Interface) {
	printStats(diskQueue)
	writeData(diskQueue)
}

func printAndWrite(diskQueue Interface) {
	printStats(diskQueue)
	time.Sleep(time.Second)
	diskQueue.Put(make([]byte, 1))
	time.Sleep(time.Second)
	printStats(diskQueue)
}
...
func TestMain(t *testing.T) {
	...
	// diskQueue, err = NewDiskqueue()

	if err == nil {
		fmt.Println("Total messages:", diskQueue.Depth())
		startWriteData(diskQueue)
	} else {
		readMetaData()
	}
}
